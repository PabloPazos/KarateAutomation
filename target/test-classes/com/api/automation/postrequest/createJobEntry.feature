Feature: To create the Job entry in the application
  ...
  Scenario: To create the Job Entry using XML request body
    Given path '/normal/webapi/add'
  ...
  Scenario: To create the Job Entry using XML request body format and recive the response in XML
    Given path '/normal/webapi/add'
  ...
  Scenario: To create the Job Entry using XML request body format with embedded expresion
    Given path '/normal/webapi/add'
    * def getJobID = function() {return Math.floor((100) * Math.random());}
    * def jobID = getJobID()
